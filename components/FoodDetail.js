...
export default function FoodDetail() {
  ...
  const [rotiName, setRotiName] = useState();
  const [name, setName] = useState('');
  const [selectedRotiId, setSelectedRotiId] = useState(null);
  ...
  useEffect(() => {
    ...
    if (selectedItem.roti && selectedItem.roti.length > 0) {
      setRotiName(selectedItem.roti[0].name);
      setSelectedRotiId(selectedItem.roti[0].id);
    }
  }, []);

  const handleRotiSelection = (roti) => {
    setRotiName(roti.name);
  };

  const handleQuantityIncrement = (item) => {
    if (rotiQuantity < item) {
      setRotiQuantity(rotiQuantity + 1);
    } else {
  ...
  const handleOrder = async () => {
    ...
    if (selectedItem.roti && selectedItem.roti.length > 0) {
      ...
      orderData.rotiName = rotiName;
      orderData.rotiQuantity = rotiQuantity;
    }
            ...
            <View>
              ...
              >
                ...
                <View style={{flexDirection: 'row'}}>
                  {selectedItem.roti.map((roti) => (
                    <TouchableOpacity
                      key={roti.id}
                      onPress={() => handleRotiSelection(roti)}
                    >
                      <Text
                        style={{
                          fontFamily: 'Nunito_400Regular',
                          ...
                          backgroundColor:
                            rotiName === roti.name ? 'black' : 'white',
                          color: rotiName === roti.name ? 'white' : 'black',
                          textAlign: 'center',
              ...
              >
                ...
                <View style={{flexDirection: 'row'}}>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      const selectedRoti = selectedItem.roti.find(
                        (roti) => roti.name === rotiName
                      );

                      if (selectedRoti) {
                        // Use the selected roti's quantity for further logic
                        const rotiQuantity = selectedRoti.quantity;
                        handleQuantityIncrement(rotiQuantity);
                      }
                    }}
                  >
                    <View
